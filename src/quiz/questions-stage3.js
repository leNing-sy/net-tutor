...
export const questions = [
  ...
  },
  {
    ...
    stage: 3,
    type: 'single-choice',
    question: '两个交换机的 Bridge ID 完全相同会发生什么？',
    options: [
      '随机选择一个作为根桥',
      'STP 协议无法工作',
      '两个都成为根桥',
    ...
    answer: 1,
    explanation: 'Bridge ID 包含 priority 和 MAC，应该全网唯一。相同会导致 STP 无法正确选举根桥',
  },
  {
    id: 'stage3-q12',
    stage: 3,
    type: 'multiple-choice',
    question: '以下哪些会触发 STP 重新计算？（多选）',
    options: [
      '收到更优的 BPDU',
      '链路断开',
      '收到普通数据帧',
    ...
    answer: [0, 1, 3],
    explanation: '收到更优 BPDU、链路变化、交换机重启都会触发 STP 重算。普通数据帧不影响 STP',
    lesson: { stage: 3, eventRange: [0, 20] },
  },
  {
    id: 'stage3-q13',
    stage: 3,
    type: 'fill-in-blank',
    question: 'STP 中，非根桥选择到根桥路径开销最小的端口作为什么端口？（英文单词）',
    answer: 'root',
    explanation: 'Root Port（根端口）是非根桥上到根桥路径开销最小的端口。参考 stp.js 的 recompute()',
  },
  {
    id: 'stage3-q14',
    stage: 3,
    type: 'true-false',
    question: 'Blocked 端口不转发数据帧，也不接收 BPDU。',
    answer: false,
    explanation: '错误。Blocked 端口不转发数据帧，但仍然接收和处理 BPDU，用于监测拓扑变化',
    lesson: { stage: 3, eventRange: [10, 30] },
  },
  {
    id: 'stage3-q15',
    stage: 3,
    type: 'scenario',
    question: '拓扑：三台交换机形成三角形（sw1-sw2-sw3-sw1）。sw1 priority=4096, sw2=8192, sw3=8192。哪个交换机是根桥？',
    topology: 'sw1 (priority=4096) -- sw2 (priority=8192) -- sw3 (priority=8192) -- sw1',
    options: [
      'sw1',
      'sw2',
      'sw3',
    ...
    answer: 0,
    explanation: 'sw1 的 priority 最小（4096），成为根桥。比较 Bridge ID 时，priority 优先级高于 MAC',
