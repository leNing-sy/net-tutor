...
export const questions = [
  ...
  },
  {
    ...
    stage: 2,
    type: 'single-choice',
    question: 'Access 端口收到带 VLAN 标签的帧会如何处理？',
    options: [
      '接受并转发',
      '剥离标签后转发',
      '丢弃该帧',
    ...
    answer: 2,
    explanation: 'Access 端口只接受无标签帧，带标签的帧会被丢弃（reason: "tagged-on-access"）。参考 vlan.js 的 ingress()',
    lesson: { stage: 2, eventRange: [0, 10] },
  },
  {
    id: 'stage2-q12',
    stage: 2,
    type: 'multiple-choice',
    question: 'Trunk 端口的 native VLAN 有什么特点？（多选）',
    options: [
      '入口时无标签帧归属到 native VLAN',
      '出口时 native VLAN 的帧不打标签',
      'native VLAN 必须是 VLAN 1',
      '可以配置为任意 VLAN ID',
    ],
    answer: [0, 1, 3],
    explanation: 'Native VLAN 是 trunk 端口对无标签帧的默认处理 VLAN，可以配置为任意 ID（不一定是 1）',
  },
  {
    id: 'stage2-q13',
    stage: 2,
    type: 'fill-in-blank',
    question: 'VLAN 标签在以太网帧中的 EtherType 是？（十六进制，如 0x0000）',
    answer: '0x8100',
    explanation: 'IEEE 802.1Q 定义的 VLAN 标签 EtherType 是 0x8100。参考 vlan.js 的 VLAN_ETHERTYPE',
  },
  {
    id: 'stage2-q14',
    stage: 2,
    type: 'true-false',
    question: 'FDB 表项的键是 (MAC, VLAN)，同一个 MAC 可以在不同 VLAN 中学到不同的端口。',
    answer: true,
    explanation: '正确。启用 VLAN 后，FDB 键变为 (address, vlan)，同一个 MAC 可以分别出现在 VLAN 10 和 VLAN 20 中',
    lesson: { stage: 2, eventRange: [10, 20] },
  },
  {
    id: 'stage2-q15',
    stage: 2,
    type: 'scenario',
    question: '拓扑：h1 (VLAN 10 access) -- sw1.p1 | sw1.p2 (trunk, native=10, allowed=[10,20]) -- sw2。h1 发送无标签帧到 sw1.p1，从 sw1.p2 出去时帧的状态是？',
    topology: 'h1 -- sw1.p1 (access, VLAN 10) | sw1.p2 (trunk, native=10)',
    options: [
      '无标签（因为是 native VLAN）',
      '带 VLAN 10 标签',
      '带 VLAN 1 标签',
    ...
    answer: 0,
    explanation: 'Trunk 端口出口时，native VLAN 的帧不打标签。h1 的帧归属 VLAN 10，恰好是 p2 的 native VLAN',
    lesson: { stage: 2, eventRange: [0, 15] },
  },
];
